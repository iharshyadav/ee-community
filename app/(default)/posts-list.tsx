...
import { fetchPosts } from "@/components/post/getposts";

export default function PostsList() {

  const {data , isFetched} = useQueryComponent(['all-post'] , fetchPosts)

  if (!isFetched) return;

  const posts = data as PostData
    ...
    <div>
        ...
        <div className="text-center md:text-left md:flex justify-between items-center">
          ...
          <ul className="grow inline-flex flex-wrap text-sm font-medium -mx-3 -my-1">
            <li className="px-3 py-1">
              <a
                className="text-green-500 relative transition duration-150 ease-in-out"
                href="#0"
              >
                <svg
                  className="fill-green-500 absolute top-full"
                  width="53"
                  height="6"
      ...
      <div className="mb-12">
                  ...
                  <div className="sm:flex items-center space-y-3 sm:space-y-0 sm:space-x-5">
                    <div className="shrink-0">
                      <button className="flex items-center text-left w-16 px-2.5 py-1 rounded border border-slate-700 bg-gradient-to-tr from-slate-800/20 via-slate-800/50 to-slate-800/20 hover:bg-slate-800 transition duration-150 ease-in-out">
                        <svg
                          className="shrink-0 fill-green-400 mr-1.5"
                          width="11"
                          height="7"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                        ...
                        <span className="grow text-center text-xs font-medium text-green-500">
                          {/* {post.votes} */}
                          22
                        </span>
                      </button>
        ...
        <div className="text-right">
          <button className="btn-sm py-1.5 h-8 text-green-500 border border-slate-700 bg-gradient-to-tr from-slate-800/20 via-slate-800/50 to-slate-800/20 hover:bg-slate-800">
