...
export default function DropdownNotifications({ align }: {
  ...
  const {messages , questionId} = useSocket();

  let uni = messages.filter((i) => i.questionId == questionId)

  console.log(uni)

  console.log(messages.filter((i) => i.questionId !== questionId));

  return (
    <Menu as="div" className="relative inline-flex w-96">
      {({ open }) => (
        <>
          <MenuButton
                ...
                {({ active }) => (
                  <Link className={`block py-2 px-4 ${active && 'bg-gray-50 dark:bg-gray-700/20'}`} href="#0">
                    <span className="block text-sm mb-2">
                      📣 <span className="font-medium text-gray-800 dark:text-gray-100">{m.questionId}</span>
                        {m.msg}
                    </span>
